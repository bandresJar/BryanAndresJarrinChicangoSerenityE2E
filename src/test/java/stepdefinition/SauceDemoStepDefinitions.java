/*package stepdefinition;

import co.com.screenplay.project.interactions.ClickOnElement;
import co.com.screenplay.project.questions.*;
import co.com.screenplay.project.tasks.*;
import co.com.screenplay.project.userinterface.CartPage;
import co.com.screenplay.project.userinterface.CheckoutPage;
import co.com.screenplay.project.userinterface.OverviewPage;
import co.com.screenplay.project.userinterface.ProductPage;
import io.cucumber.java.Before;
import io.cucumber.java.en.*;

import net.serenitybdd.screenplay.Actor;
import net.serenitybdd.screenplay.abilities.BrowseTheWeb;
import net.serenitybdd.screenplay.actors.Cast;
import net.serenitybdd.screenplay.actors.OnStage;
import net.serenitybdd.screenplay.waits.WaitUntil;
import net.thucydides.core.webdriver.SerenityWebdriverManager;

import static net.serenitybdd.screenplay.GivenWhenThen.seeThat;
import static net.serenitybdd.screenplay.matchers.WebElementStateMatchers.isVisible;
import static org.hamcrest.Matchers.is;

public class SauceDemoStepDefinitions {

    @Before
    public void setUp() {
        // Inicializar el escenario y el actor
        OnStage.setTheStage(new net.serenitybdd.screenplay.actors.Stage(new Cast()));
        Actor usuario = OnStage.theActorCalled("Usuario"); // Nombra al actor

        // Permitir que el actor navegue por la web
        usuario.can(BrowseTheWeb.with(SerenityWebdriverManager.inThisTestThread().getCurrentDriver()));
    }

    @Given("el usuario esta en la pagina de inicio de sesion")
    public void elUsuarioEstaEnLaPaginaDeInicioDeSesion() {
        OnStage.theActorInTheSpotlight().attemptsTo(NavigateToLoginPage.to("https://www.saucedemo.com/"));
    }

    @When("el usuario ingresa el nombre de usuario {string} y la contrasena {string}")
    public void elUsuarioIngresaCredenciales(String username, String password) {
        OnStage.theActorInTheSpotlight().attemptsTo(Login.withCredentials(username, password));
    }

    @Then("deberia ver la pagina principal con el titulo {string}")
    public void deberiaVerLaPaginaPrincipalConTitulo(String expectedTitle) {
        OnStage.theActorInTheSpotlight().should(seeThat(IsTitleVisible.on(ProductPage.TITLE), is(true)));
    }

    /*@Given("el usuario ha iniciado sesion en la pagina principal")
    public void elUsuarioHaIniciadoSesionEnLaPaginaPrincipal() {
        OnStage.theActorInTheSpotlight().attemptsTo(
                WaitUntil.the(ProductPage.TITLE, isVisible())
        );
        OnStage.theActorInTheSpotlight().should(seeThat(IsTitleVisible.on(ProductPage.TITLE), is(true)));

    }

    @When("el usuario agrega dos productos al carrito")
    public void elUsuarioAgregaDosProductosAlCarrito() {
        OnStage.theActorInTheSpotlight().attemptsTo(
                AddProductsToCart.addTwoRandomItems()
        );
    }

    @When("el usuario visualiza el carrito")
    public void elUsuarioVisualizaElCarrito() {
        // Write code here that turns the phrase above into concrete actions
        OnStage.theActorInTheSpotlight().attemptsTo(
                ViewCart.itemsInCart());
    }

    /*@Then("deberia ver que el carrito contiene exactamente {int} productos")
    public void deberiaVerQueElCarritoContieneExactamenteProductos(int expectedItemCount) {
        OnStage.theActorInTheSpotlight().should(seeThat(CartItemsCount.is(), is(expectedItemCount)));
    }
    @Then("dar clic en boton Checkout")
    public void darClicEnBotonCheckout() {
        // Write code here that turns the phrase above into concrete actions
        OnStage.theActorInTheSpotlight().attemptsTo(FinalizePurchase.orderCompletion());
    }




    @Given("el usuario esta en la pagina de resumen de compra")
    public void elUsuarioEstaEnLaPaginaDeResumenDeCompra() {
        // Verificar que el botón de checkout está visible
        OnStage.theActorInTheSpotlight().should(seeThat(IsCheckoutButtonVisible.on(CheckoutPage.CONTINUE_BUTTON), is(true)));
    }

    @When("el usuario llena datos del formulario")
    public void elUsuarioLlenaDatosDelFormulario() {
        // Llenar los datos del formulario
        OnStage.theActorInTheSpotlight().attemptsTo(CompletePurchaseForm.withDetails("Andres", "Jarrin", "176178"));


    }

    @When("el usuario da clic en el boton Continue")
    public void elUsuarioDaClicEnElBotonContinue() {
        // Dar clic en el botón Continue
        OnStage.theActorInTheSpotlight().attemptsTo(ClickContinueButton.on());

    }

    @Then("el usuario deberia ver la pagina de resumen de compra")
    public void elUsuarioDeberiaVerLaPaginaDeResumenDeCompra() {
        // Verificar que se muestra la página de resumen de compra
        OnStage.theActorInTheSpotlight().should(seeThat(IsCheckoutButtonVisible.on(CheckoutPage.FINISH_BUTTON), is(true)));

    }




    @Given("el usuario esta en la pagina de compra")
    public void elUsuarioEstaEnLaPaginaDeCompra() {
        OnStage.theActorInTheSpotlight().should(seeThat(IsCheckoutButtonVisible.on(OverviewPage.FINISH_BUTTON), is(true)));
    }

    @When("el usuario da clic en Finalize")
    public void elUsuarioDaClicEnFinalize() {
        OnStage.theActorInTheSpotlight().attemptsTo(ClickFinishButton.on());
    }

    @Then("deberia ver el mensaje de confirmacion {string}")
    public void deberiaVerElMensajeDeConfirmacion(String expectedMessage) {
        OnStage.theActorInTheSpotlight().should(seeThat(ConfirmationMessage.visible(), is(expectedMessage)));
    }
}*/

package stepdefinition;

import co.com.screenplay.project.questions.*;
import co.com.screenplay.project.tasks.*;
import co.com.screenplay.project.userinterface.ProductPage;
import io.cucumber.java.Before;
import io.cucumber.java.en.*;

import net.serenitybdd.screenplay.Actor;
import net.serenitybdd.screenplay.abilities.BrowseTheWeb;
import net.serenitybdd.screenplay.actors.Cast;
import net.serenitybdd.screenplay.actors.OnStage;
import net.thucydides.core.webdriver.SerenityWebdriverManager;

import static net.serenitybdd.screenplay.GivenWhenThen.seeThat;
import static org.hamcrest.Matchers.is;

public class SauceDemoStepDefinitions {

    @Before
    public void setUp() {
        // Inicializa el escenario y el actor
        OnStage.setTheStage(new net.serenitybdd.screenplay.actors.Stage(new Cast()));
        Actor usuario = OnStage.theActorCalled("Usuario"); // Nombra al actor

        // Permitir que el actor navegue por la web
        usuario.can(BrowseTheWeb.with(SerenityWebdriverManager.inThisTestThread().getCurrentDriver()));
    }

    @Given("el usuario esta en la pagina de inicio de sesion")
    public void elUsuarioEstaEnLaPaginaDeInicioDeSesion() {
        OnStage.theActorInTheSpotlight().attemptsTo(NavigateToLoginPage.to("https://www.saucedemo.com/"));
    }

    @When("el usuario ingresa el nombre de usuario {string} y la contrasena {string}")
    public void elUsuarioIngresaCredenciales(String username, String password) {
        OnStage.theActorInTheSpotlight().attemptsTo(Login.withCredentials(username, password));
    }

    @Then("deberia ver la pagina principal con el titulo {string}")
    public void deberiaVerLaPaginaPrincipalConTitulo(String expectedTitle) {
        OnStage.theActorInTheSpotlight().should(seeThat(IsTitleVisible.on(ProductPage.TITLE), is(true)));
    }

    @When("el usuario agrega dos productos al carrito")
    public void elUsuarioAgregaDosProductosAlCarrito() {
        OnStage.theActorInTheSpotlight().attemptsTo(AddProductsToCart.addTwoRandomItems());
    }

    @When("el usuario revisa el carrito")
    public void elUsuarioRevisaElCarrito() {
        OnStage.theActorInTheSpotlight().attemptsTo(ViewCart.itemsInCart());
    }

    @Then("el usuario acepta los productos en el carrito")
    public void elUsuarioAceptaLosProductosEnElCarrito() {
        OnStage.theActorInTheSpotlight().attemptsTo(FinalizePurchase.orderCompletion());
    }

    @When("el usuario llena los datos de envio")
    public void elUsuarioLlenaLosDatosDeEnvio() {
        OnStage.theActorInTheSpotlight().attemptsTo(CompletePurchaseForm.withDetails("Andres", "Jarrin", "176178"));
    }

    @When("el usuario acepta la compra")
    public void elUsuarioAceptaLaCompra() {
        // Implementar lógica para revisar la compra antes de finalizar.
        OnStage.theActorInTheSpotlight().attemptsTo(ClickContinueButton.on());
    }

    @Then("el usuario finaliza la compra")
    public void elUsuarioFinalizaLaCompra() {
        OnStage.theActorInTheSpotlight().attemptsTo(ClickFinishButton.on());
    }

    @Then("el usuario deberia ver el mensaje de confirmacion {string}")
    public void deberiaVerElMensajeDeConfirmacion(String expectedMessage) {
        OnStage.theActorInTheSpotlight().should(seeThat(ConfirmationMessage.visible(), is(expectedMessage)));
    }
}
